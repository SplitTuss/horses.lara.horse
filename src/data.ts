export const HORSE_DATA = [
  {
    name: 'Lovis',
    age: '16 years old',
    breed: 'Arabian-Welsh Pony mix, mare',
    timeline: 'Feb 2013 - Nov 2013',
    notes:
      "Lovis was a school pony and hated everyone until someone (I) took the time to build her trust and train her. Obviously I had help from people who knew what they were doing. Half a year after I started working with her, I showed her in the Trail, Western Pleasure and Horsemanship classes. How we got placed didn't matter, we did it for the experience and learning. I remember it being quite successful but don't remember the exact places we got. A few weeks later she got sold. I tried to stay in touch but the new owner wasn't a fan and it wasn't the same anymore, so I had to move on.",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/lovis/lovis2.JPG',
        alt: 'This is Lovis',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/lovis/lovis3.JPG',
        alt: 'This was Lovis and me in 2013, at both our first show in the Western Pleasure class.',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/lovis/lovis1.jpg',
        alt: 'She was such a furball in winter.',
      },
    ],
  },
  {
    name: 'Oz',
    age: '10 years old',
    breed: 'Andalusian-Knabstrupper mix, gelding',
    timeline: 'Nov 2013 - Oct 2014',
    notes:
      "He was actually the first horse on that farm I bonded with, it just wasn't as close of a bond as I had with Lovis a couple of years later. When I first started riding there, they hosted a tiny Trail-tournament for all the kids. I tried with Oz and did pretty good, considering I barely knew how to ride and barely knew the horse. A few years later they did the same thing and I also did that one with him because he was such a reliable and fun horse. You could do anything with him, even spray paint him (with safe paint and adult's supervision of course). When I went to visit the farm YEARS later, him and his brother Jasko were still there, which made me super happy. ",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/oz/oz1.jpg',
        alt: 'It was pretty muddy that day but that just meant we didn`t use the saddle, so it stayed clean.',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/oz/oz2.JPG',
        alt: 'We dressed Oz up for carnival. He really didn`t mind the shenanigans, neither did the chickens in the background.',
      },
    ],
  },
  {
    name: 'Jespers Fiasko, aka Jasko',
    age: '10 years old',
    breed: 'Knabstrupper, gelding',
    timeline: 'Nov 2013 - Oct 2014',
    notes:
      "Oz' brother, who was bred on accident but he was one of the most stunning 'accidents' I have ever met. After Lovis was sold, he was my go-to mount. He taught me a lot of things, including how to jump onto a tall horses' back without a saddle or tree stump. Knabstruppers are known for the tricks they like participating in, so he knew A LOT of them. One time, he accidentally kicked me in the knee and sent me flying 4 meters through the air. There was a bee on his belly and it wouldn't go away and I walked around him right behind him, which you're not supposed to do for a reason. When Jasko saw what happened, you could tell that he felt bad. He was a wonderful companion and I am grateful to have bonded with him the way that I did. When I went to visit the farm YEARS later, him and his brother Oz were still there, which made me super happy. ",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/jasko/jasko1.jpg',
        alt: 'Jasko and me in a puddle',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/jasko/jasko2.JPG',
        alt: 'Jasko and me, as usual, without saddle in an open field.',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/jasko/jasko3.JPG',
        alt: "This is Jasko's face, look at his eyeliner!",
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/jasko/jasko4.png',
        alt: 'This is one way to mount a tall horse, have them take a bow!',
      },
    ],
  },
  {
    name: 'Tralee',
    age: '12 years old',
    breed: 'Irish Cob, gelding',
    timeline: 'Oct 2014 - 2017',
    notes:
      "At some point the school horses (Oz and Jasko) couldn't teach me much more, so I had to find a new stable with someone's private horse that I got to ride. So I did. I had the honor of working with Tralee for two years before he too was sold, for reasons that to this day upset me. He wasn't my horse though so there's nothing I could do. I was able to ride Tralee without a saddle and without a bridle and he would still be perfect. I could ride such small lope circles that it was almost a piaffe, with no bridle and no saddle, of course. I jumped small obstacles with him even though he was never trained for jumping. I went on many trail rides with him, even some in the dark where I was pretty sure I was lost but he never was, he found home. I could go on trail rides (obviously) without saddle or bridle. I got to show him once, in a Western pleasure. Shortly after that they sold him.",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/tralee/tralee1.JPG',
        alt: 'To this day one of my favorite photos of Tralee and me.',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/tralee/tralee2.jpg',
        alt: 'Tralee and me in a field',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/tralee/tralee3.JPG',
        alt: 'Tralee <3',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/tralee/tralee4.JPG',
        alt: 'This was in 2015, my first Western Pleasure show with Tralee.',
      },
    ],
  },
  {
    name: 'Pico, aka Pixel',
    age: '14 years old',
    breed: 'Quarter Horse-Wels Pony mix, gelding',
    timeline: '2017 - 2019',
    notes:
      "His owner offered me to work with him when Tralee was sold. I had the choice between Tralee's owner's new horse (a 5- year old mare called Diva, who tried to bite my leg every time I got on her) or Pixel. You can tell who I went for. He was amazing but I had to use the reins a lot with him, which I wasn't a big fan of but that was no fault of his owner. She worked with him on needing less rein-cues but coming from Tralee who needed none at all, it was a big change. He was a super kind horse and his owner supported me in ways that no other horse owner had ever done. It was a real team. Until life got in the way and I decided to give up on riding for a while. ",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/pico/pico1.jpg',
        alt: 'This is Pico',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/pico/pico2.JPG',
        alt: "Pico and me walking to the stable. He's a short but very lovable guy",
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/pico/pico3.jpg',
        alt: 'Pico has a beautiful face too',
      },
    ],
  },
  {
    name: 'Tiana',
    age: '7 years old',
    breed: 'Haflinger, mare',
    timeline: '2020',
    notes:
      "Her owner was very sick, so she had 8 people taking care of Tiana and everyone did it in their own way. Again, I got to work with a horse who is confused by too many people (only this time she was in puberty too) but people had messed up too much with her, so there was little I could do, as long as random people kept working with her too. In the round pen she came running at me with her ears pinned back and her teeth fletched at me. She'd rear when I wouldn't let her run home at the end of our trail rides. I didn't have the time to work with her every day, otherwise I may have other things to say. I had nobody working with me, everyone felt bad for the owner and horse so everyone wanted to help. Sadly nobody took the time to really get to know Tiana and work with the other people caring for her, so that she'd have people doing similar things. She was a confused teen who just wanted to be cuddled all day every day. They found a replacement for me and that person got to keep her after the original owner had passed away a few months after I left. ",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/tiana/tiana1.jpg',
        alt: 'Tiana in her stall',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/tiana/tiana2.jpg',
        alt: 'Tiana with a braided mane',
      },
    ],
  },
  {
    name: 'Nala',
    age: '8 years old',
    breed: 'unsure about breed, mare',
    timeline: 'Apr 2023 - Jul 2023',
    notes:
      "I didn't get to work with her as much as I would've liked. She had really bad hooves and that gave her only very little mobility, so working with her was pretty much impossible. I took her for walks, that was all she could do, especially since nobody really did much with her so her muscles were almost non-existent. Such a sweet girl though, she really listened to body language and whenever I did get to ride her for a little bit (without a saddle, as I like to do), it was such a joy. Nala also reacted to the reins so sensitively that she didn't need a bridle, I just used her halter and she was golden.",
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/nala/nala1.jpeg',
        alt: "This is Nala. The bright pink halter doesn't help with the cuteness",
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/nala/nala2.jpeg',
        alt: 'Nala in all her tinyness',
      },
    ],
  },
  {
    name: 'Fuchur',
    age: '14 years old',
    breed: 'Noriker, gelding',
    timeline: 'Mar 2023- Jan 2024',
    notes:
      'Sadly nobody ever taught this huge horse respect or personal space, which led to no trust and no control whatsoever, which long story short, was the reason why I left. It was very similar to Tiana, only Fuchur`s owner was not sick. There was 4 people taking care of him, again, everyone doing what they thought was best, or not thinking about it at all. Fuchur was a great trail riding horse, as long as you`d want nothing from him besides follow the trail and figure out nature`s challenges. If you wanted to go a different way though or have him stop or even back him up, that was impossible. I do not even know what to say about a horse owner who doesn`t at some point within the 14 YEARS of owning him, teach their horse to back up.',
    images: [
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/fuchur/fuchur2.jpg',
        alt: "Fuchur and me riding up a steep hill with Gioia, our friends' dog",
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/fuchur/fuchur3.jpg',
        alt: 'Fuchur and me walking to the stable. He was a tall guy.',
      },
      {
        url: 'https://s3.us-east-1.amazonaws.com/images.horses.lara.horse/fuchur/fuchur1.JPG',
        alt: 'Fuchur and his best friend Jimmy',
      },
    ],
  },
];
